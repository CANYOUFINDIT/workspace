    ...
    def login(self, uid, password):
        while True:
            ...
            if request.status_code != requests.codes.ok:
                ...
                time.sleep(0.5)
                continue
            if request.text.find('Code error,please input again') > -1:
                print('Code error,please input again')
                continue
            if request.text.find('Password may be error') > -1:
                print('Password may be error')
                return False
            if request.text.find('Uid may be error') > -1:
...
if __name__ == '__main__':
    url = 'http://jwgl1.hbnu.edu.cn/(S(os5nraurrzuankqdkm424cnz))/default2.aspx'
    spider = Spider(url)
    print('请输入学号')
    uid = input()  #学号
    print('请输入密码')
